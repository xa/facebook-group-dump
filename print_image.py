...
def print_image(image, scale=1, margin=0, truecolor=False):
	...
	for y in range(h):
		...
		for x in range(w):
			...
			try:
				...
				r2, g2, b2 = (r1, g1, b1)
			if truecolor:
				sys.stdout.write("\033[38;2;"+str(r1)+";"+str(g1)+";"+str(b1)+"m\033[48;2;"+str(r2)+";"+str(g2)+";"+str(b2)+"m▀")
			else:
				short1 = rgb2short("%02x%02x%02x" % (r1, g1, b1))
				short2 = rgb2short("%02x%02x%02x" % (r2, g2, b2))
				sys.stdout.write("\033[38;5;"+short1+"m\033[48;5;"+short2+"m▀")
		sys.stdout.write("\033[0m\n")
	sys.stdout.write("\n")

def print_bytes(bytes, scale=1, margin=0, truecolor=False):
	if is_ios():
		return
	image = Image.open(io.BytesIO(bytes))
	print_image(image, scale=scale, margin=margin, truecolor=truecolor)

def print_url(url, scale=1, margin=0, truecolor=False):
	import requests
	bytes = requests.get(url).content
	print_bytes(bytes, scale=scale, margin=margin, truecolor=truecolor)

if __name__ == '__main__':
	print_url("https://thispersondoesnotexist.com/image")
